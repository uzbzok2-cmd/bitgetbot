...
def generate_trendline_break_chart(candles_data: list, pat: dict) -> io.BytesIO:
    """🔷 Trend Buzish — faqat trend chizig'i + breakout + body-based TP/SL."""
    timestamps, opens, highs, lows, closes, vols = _make_candles(candles_data)
    if not closes:
        return _empty_chart(pat.get("symbol", "?"))
    ...
    conf      = pat["confidence"]
    window    = pat.get("window", 80)
    trend_ln  = pat["trend_line"]
    break_idx = pat["break_idx"]
    tp_levels = pat.get("tp_levels", [tp])

    BG      = "#0d1117"; GRID   = "#1f2937"
    UP_BODY = "#26a69a"; DN_BODY = "#ef5350"
    WICK    = "#6b7280"; TEXT_C = "#e5e7eb"
    ENTRY_C = "#f59e0b"; TP_C   = "#22c55e"
    SL_C    = "#ef4444"; TREND_C= "#fb923c"
    RES_C   = "#f87171"
    ...
    # Trend chizig'i (pasayuvchi diagonal)
    x0c = trend_ln["x0"]
    x1c = n - 1
    y0  = trend_ln["y0"]
    y1  = trend_ln["slope"] * x1c + trend_ln["intercept"]
    ax.plot([x0c, x1c], [y0, y1], color=TREND_C, linewidth=1.8, linestyle="--",
            alpha=0.9, zorder=3)

    # Breakout nuqtasi (oq doira)
    break_x = break_idx - offset
    if 0 <= break_x < n:
        ax.scatter([break_x], [cl_n[break_x]], s=140, facecolors="none",
                   edgecolors="white", linewidths=2.0, zorder=6)
        ax.annotate("🔺 Breakout", (break_x, cl_n[break_x]),
                    textcoords="offset points", xytext=(0, 14),
                    color="white", fontsize=8, fontweight="bold", ha="center")

    # TP zonalari (tana asosida, 1-2 ta)
    for i, lvl in enumerate(tp_levels):
        ax.axhline(lvl, color=RES_C if i > 0 else TP_C, linewidth=1.3,
                   linestyle="--", alpha=0.8)
        ax.text(x_right, lvl, f" TP{i+1} ${_fmt(lvl)}",
                color=RES_C if i > 0 else TP_C, fontsize=7.5,
                va="center", fontweight="bold")

    ax.axhline(entry, color=ENTRY_C, linewidth=1.6, linestyle="-", alpha=0.95)
    ax.text(x_right, entry, f" ⚡ Kirish ${_fmt(entry)}", color=ENTRY_C,
            fontsize=7.5, va="center", fontweight="bold")

    ax.axhline(sl, color=SL_C, linewidth=1.4, linestyle="--", alpha=0.85)
    ax.text(x_right, sl, f" 🛑 SL ${_fmt(sl)}", color=SL_C,
            fontsize=7.5, va="center", fontweight="bold")

    ax.axhspan(entry, tp, alpha=0.06, color=TP_C)
    ax.axhspan(sl, entry, alpha=0.06, color=SL_C)

    all_prices = list(hi_n) + list(lo_n) + tp_levels + [entry, sl]
    margin = (max(all_prices) - min(all_prices)) * 0.14
    ax.set_xlim(-1, x_right + 20)
    ax.set_ylim(min(all_prices) - margin, max(all_prices) + margin)

    ax.set_title(
        f"{symbol}  •  {timeframe}  |  🔷 Trend Buzish (Trendline Break)  "
        f"•  🟢 LONG  •  {conf}% ishonch",
    ...
    import matplotlib.patches as mpatches
    patches = [
        mpatches.Patch(color=TREND_C, label="Trend chizig'i"),
        mpatches.Patch(color=ENTRY_C, label=f"Kirish ${_fmt(entry)}"),
        mpatches.Patch(color=TP_C,    label=f"TP (tana) ${_fmt(tp)}"),
